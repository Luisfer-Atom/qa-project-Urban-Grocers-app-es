...
def test3_create_kit_without_name_negative_response():
    current_kit_body = get_kit_body(data.test3_kit_name)
    negative_assert_code_400(current_kit_body)

def test4_create_kit_512_letter_in_the_name_negative_response():
    current_kit_body = get_kit_body(data.test4_kit_name)
    negative_assert_code_400(current_kit_body)

def test5_create_kit_special_character_in_the_name_success_response():
    current_kit_boddy = get_kit_body(data.test5_kit_name)
    positive_assert(current_kit_boddy)

def test6_create_kit_with_space_in_the_name_success_response():
    current_kit_boddy = get_kit_body(data.test6_kit_name)
    positive_assert(current_kit_boddy)

def test7_create_kit_with_numbers_success_response():
    current_kit_boddy = get_kit_body(data.test7_kit_name)
    positive_assert(current_kit_boddy)
...
def test9_create_kit_not_string_parameter_negative_response():
